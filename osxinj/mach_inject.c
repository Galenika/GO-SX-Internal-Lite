            ...
            vm_size_t				stackSize )
{
	...
	if( !err )
		vm_allocate( remoteTask, &remoteCode, imageSize, 1 );
	...
	if( !err ) {
		...
		err = vm_write( remoteTask, remoteCode, (pointer_t) image, (mach_msg_size_t)imageSize );
	...
	if( !err && paramBlock != NULL && paramSize ) {
		...
		if( !err ) {
			...
			err = vm_write( remoteTask, remoteParamBlock,
                           (pointer_t) paramBlock, (mach_msg_size_t)paramSize );
		}
	}
...
#define STACK_CONTENTS_SIZE ((1+PARAM_COUNT) * sizeof(unsigned long long))
		...
		// push stackContents
		vm_write( remoteTask, remoteStack,
                       (pointer_t) stackContents, STACK_CONTENTS_SIZE);
